...
export default function FooterSection({ config }: FooterSectionProps) {
  ...
  const shopName = sanitizeText((config.shopName as string) || 'My Store');
  const tagline = sanitizeText((config.tagline as string) || 'Your trusted neighborhood shop, serving you with quality and trust since 2024.');
  const social = (config.social as Record<string, string>) || {};

  const whatsappUrl = sanitizeUrl(social.whatsapp || '#');
  const instagramUrl = sanitizeUrl(social.instagram || '#');
  const facebookUrl = sanitizeUrl(social.facebook || '#');
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              {social.whatsapp !== undefined && (
                <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" style={{ width: '48px', height: '48px', borderRadius: '14px', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              {social.instagram !== undefined && (
                <a href={instagramUrl} target="_blank" rel="noopener noreferrer" style={{ width: '48px', height: '48px', borderRadius: '14px', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              {social.facebook !== undefined && (
                <a href={facebookUrl} target="_blank" rel="noopener noreferrer" style={{ width: '48px', height: '48px', borderRadius: '14px', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', border: '1px solid rgba(255,255,255,0.1)' }}>
