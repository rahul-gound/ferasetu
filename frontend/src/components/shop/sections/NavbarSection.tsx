    ...
    <>
      ...
      >
        ...
        <div className="fera-nav-links">
          {navItems.map((link, i) => (
            link.action === 'track' ? (
              <button key={i} className="fera-nav-link" onClick={() => window.dispatchEvent(new CustomEvent('fera-open-track-order'))} style={{
        ...
        >
          {menuOpen ? (
            ...
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="3" cy="12" x2="21" y2="12"></line><line x1="3" cy="6" x2="21" y2="6"></line><line x1="3" cy="18" x2="21" y2="18"></line></svg>
          )}
        </button>
      </nav>
      ...
      {menuOpen && (
        <div style={{
          ...
          {navItems.map((link, i) => (
             link.action === 'track' ? (
                <button key={i} onClick={() => { setMenuOpen(false); window.dispatchEvent(new CustomEvent('fera-open-track-order')); }} style={{
                  display: 'block', color: '#0f172a', textDecoration: 'none', background: 'none', border: 'none', textAlign: 'left', padding: 0,
