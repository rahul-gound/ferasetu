// FeraSetu — Canonical Storefront URL & Hostname Generator (TypeScript Backend)
// ===========================================================================
...
export function generateCanonicalStorefront(input: StorefrontInput): StorefrontResult {
  const domain = (input.baseDomain || process.env.BASE_DOMAIN || DEFAULT_BASE_DOMAIN).toLowerCase().trim();
...
export async function findNextAvailableStorefront(
  ...
  let input: StorefrontInput;
  let existingChecker: any;
  ...
  input = input || { shopName: 'store' };

  const checkIsTaken = async (subdomain: string, hostname: string): Promise<boolean> => {
